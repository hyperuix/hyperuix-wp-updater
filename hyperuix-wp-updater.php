...
Text Domain: hyperuix-wp-updater
License: GPL3+
*/

// Only load class if it hasn't already been loaded
if ( !class_exists( 'HyperuixWPUpdater' ) ) {

	// Hyperuix Updater
	class HyperuixWPUpdater {
		...
		private function init() {
			...
			load_plugin_textdomain( 'hyperuix-wp-updater', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
			// Add Filters
        ...
        public function create_monthly_schedule($schedules) {
            if (!isset($schedules['hyperuix_monthly'])) {
                $schedules['hyperuix_monthly'] = array(
                    'interval' => 86400 * 30,
		...
		public function plugin_action_links( $links, $file ) {
			if ( $file == plugin_basename( __FILE__ ) ) {
				$settings_link = '<a href="' . admin_url( 'options-general.php?page=hyperuix-wp-updater' ) . '">' . __( "Settings", "hyperuix-wp-updater" ) . '</a>';
		...
		public function do_update_check($print_to_screen = false) {
			...
			if ( $core_updated || $plugins_updated || $themes_updated ) { // Did anything come back as need updating?
			    //$message = __( "There are updates available for your WordPress site:", "hyperuix-wp-updater" ) . "\n" . $message . "\n";
				//$message .= sprintf( __( "Please visit %s to update.", "hyperuix-wp-updater" ), admin_url( 'update-core.php' ) );
		...
		private function core_update_check( &$message ) {
			...
			if ( 'upgrade' == $update_core->updates[0]->response ) { // is WP core update available?
				if ( $update_core->updates[0]->current != $settings['notified']['core'] ) { // have we already notified about this version?
					...
					$message .= "\n" . sprintf( __( "WP-Core: WordPress is out of date. Please update from version %s to %s", "hyperuix-wp-updater" ), $old_core_ver, $new_core_ver ) . "\n";
			...
			if ( !empty( $update_plugins->response ) ) { // any plugin updates available?
				...
				if ( count( $plugins_need_update ) >= 1 ) { // any plugins need updating after all the filtering gone on above?
					...
					foreach ( $plugins_need_update as $key => $data ) { // loop through the plugins that need updating
						...
						$message .= "\n" . sprintf( __( "Plugin: %s is out of date. Please update from version %s to %s", "hyperuix-wp-updater" ), $plugin_info['Name'], $plugin_info['Version'], $data->new_version ) . "\n";
						$message .= "\t" . sprintf( __( "Details: %s", "hyperuix-wp-updater" ), $data->url ) . "\n";
						$message .= "\t" . sprintf( __( "Changelog: %s%s", "hyperuix-wp-updater" ), $data->url, "changelog/" ) . "\n";
						...
						$message .= "\t" . sprintf( __( "Compatibility: %s", "hyperuix-wp-updater" ), $compat ) . "\n";
			...
			if ( !empty( $update_themes->response ) ) { // any theme updates available?
				...
				if ( count( $themes_need_update ) >= 1 ) { // any themes need updating after all the filtering gone on above?
					foreach ( $themes_need_update as $key => $data ) { // loop through the themes that need updating
						...
						$message .= "\n" . sprintf( __( "Theme: %s is out of date. Please update from version %s to %s", "hyperuix-wp-updater" ), $theme_info['Name'], $theme_info['Version'], $data['new_version'] ) . "\n";
		...
		public function send_notification_email( $message ) {
			...
			$settings = $this->getSetOptions( self::OPT_FIELD ); // get settings
			$subject = __( "We've scanned your site, here's what we found", "hyperuix-wp-updater" );
			$headers = [];
			$headers[] = 'Reply-To: HYPERUIX <support@hyperuix.com.au>';
		...
		public function puw_check() {
			...
			die( __( "Successfully checked for updates.", "hyperuix-wp-updater" ) );
		}
		...
		public function admin_settings_menu() {
			$page = add_options_page( 'Update Watcher', 'Update Watcher', 'manage_options', 'hyperuix-wp-updater', array( $this, 'settings_page' ) );
		...
		public function settings_page() {
			...
			?>
			<div class="wrap">
				<h2><?php _e( "Update Watcher", "hyperuix-wp-updater" ); ?></h2>

				<p>
                    <span class="description">
                    <?php
					if ( false === $options["last_check_time"] ) {
						$scan_date = __( "Never", "hyperuix-wp-updater" );
					}
					else {
						$scan_date = sprintf(
							__( "%1s @ %2s", "hyperuix-wp-updater" ),
							date( $date_format, $options["last_check_time"] ),
							date( $time_format, $options['last_check_time'] )
						);
					}

					echo sprintf( __( "Last scanned: %s", "hyperuix-wp-updater" ), $scan_date );
					?>
                    </span>
				</p>
				...
				<form action="<?php echo admin_url( "options.php" ); ?>" method="post">
					...
					do_settings_sections( "hyperuix-wp-updater" );
					?>
					<p>&nbsp;</p>
					<input class="button-primary" name="Submit" type="submit" value="<?php _e( "Save settings", "hyperuix-wp-updater" ); ?>" />
                    &nbsp;&nbsp;|&nbsp;&nbsp;
					<input class="button" name="submitwithemail" type="submit" value="<?php _e( "Save settings &amp; Send email now", "hyperuix-wp-updater" ); ?>" />
					<input class="button" name="submitwithprinttoscreen" type="submit" value="<?php _e( "Save settings &amp; Show all updates", "hyperuix-wp-updater" ); ?>" />
				</form>
			</div>
		<?php
		}
		...
		public function admin_settings_init() {
			...
			add_settings_section( "puw_settings_main", __( "Settings", "hyperuix-wp-updater" ), array( $this, "puw_settings_main_text" ), "hyperuix-wp-updater" ); // Make settings main section
			add_settings_field( "puw_settings_main_cron_method", __( "Cron Method", "hyperuix-wp-updater" ), array( $this, "puw_settings_main_field_cron_method" ), "hyperuix-wp-updater", "puw_settings_main" );
			add_settings_field( "puw_settings_main_frequency", __( "Frequency to check", "hyperuix-wp-updater" ), array( $this, "puw_settings_main_field_frequency" ), "hyperuix-wp-updater", "puw_settings_main" );
			add_settings_field( "puw_settings_main_notify_to", __( "Notify email to", "hyperuix-wp-updater" ), array( $this, "puw_settings_main_field_notify_to" ), "hyperuix-wp-updater", "puw_settings_main" );
			add_settings_field( "puw_settings_main_notify_from", __( "Notify email from", "hyperuix-wp-updater" ), array( $this, "puw_settings_main_field_notify_from" ), "hyperuix-wp-updater", "puw_settings_main" );
			add_settings_field( "puw_settings_main_notify_to_name", __( "Recipient name", "hyperuix-wp-updater" ), array( $this, "puw_settings_main_field_notify_to_name" ), "hyperuix-wp-updater", "puw_settings_main" );
			add_settings_field( "puw_settings_main_notify_plugins", __( "Notify about plugin updates?", "hyperuix-wp-updater" ), array( $this, "puw_settings_main_field_notify_plugins" ), "hyperuix-wp-updater", "puw_settings_main" );
			add_settings_field( "puw_settings_main_notify_themes", __( "Notify about theme updates?", "hyperuix-wp-updater" ), array( $this, "puw_settings_main_field_notify_themes" ), "hyperuix-wp-updater", "puw_settings_main" );
			add_settings_field( "puw_settings_main_notify_automatic", __( "Notify automatic core updates to this address?", "hyperuix-wp-updater" ), array( $this, "puw_settings_main_field_notify_automatic" ), "hyperuix-wp-updater", "puw_settings_main" );
			add_settings_field( "puw_settings_main_hide_updates", __( "Hide core WP update nag from non-admin users?", "hyperuix-wp-updater" ), array( $this, "puw_settings_main_field_hide_updates" ), "hyperuix-wp-updater", "puw_settings_main" );
		}
		...
		public function puw_settings_validate( $input ) {
			...
			if ( isset( $input['cron_method'] ) && in_array( $input['cron_method'], array( "wordpress", "other" ) ) ) {
				...
				add_settings_error( "puw_settings_main_cron_method", "puw_settings_main_cron_method_error", __( "Invalid cron method selected", "hyperuix-wp-updater" ), "error" );
			}
			...
			if ( in_array( $input['frequency'], $this->get_intervals() ) ) {
				...
				add_settings_error( "puw_settings_main_frequency", "puw_settings_main_frequency_error", __( "Invalid frequency entered", "hyperuix-wp-updater" ), "error" );
			}
			...
			if ( $emails_to ) {
				...
				foreach ( $emails_to as $email_to ) {
					...
					if ( !is_email( $address ) ) {
						add_settings_error( "puw_settings_main_notify_to", "puw_settings_main_notify_to_error", __( "One or more email to addresses are invalid", "hyperuix-wp-updater" ), "error" );
						$was_error = true;
						break;
					}
				...
				add_settings_error( "puw_settings_main_notify_to", "puw_settings_main_notify_to_error", __( "No email to address entered", "hyperuix-wp-updater" ), "error" );
			}
			...
			if ( is_email( $sanitized_email_from ) ) {
				...
				add_settings_error( "puw_settings_main_notify_from", "puw_settings_main_notify_from_error", __( "Invalid email from entered", "hyperuix-wp-updater" ), "error" );
			}
			...
			if ( !empty( $sanitized_email_to_name ) ) {
				...
				add_settings_error( "puw_settings_main_notify_to_name", "puw_settings_main_notify_to_name", __( "Invalid recipient name", "hyperuix-wp-updater" ), "error" );
			}
			...
			if ( $sanitized_notify_plugins >= 0 && $sanitized_notify_plugins <= 2 ) {
				...
				add_settings_error( "puw_settings_main_notify_plugins", "puw_settings_main_notify_plugins_error", __( "Invalid plugin updates value entered", "hyperuix-wp-updater" ), "error" );
			}
			...
			if ( $sanitized_notify_themes >= 0 && $sanitized_notify_themes <= 2 ) {
				...
				add_settings_error( "puw_settings_main_notify_themes", "puw_settings_main_notify_themes_error", __( "Invalid theme updates value entered", "hyperuix-wp-updater" ), "error" );
			}
			...
			if ( $sanitized_notify_automatic >= 0 && $sanitized_notify_automatic <= 1 ) {
				...
				add_settings_error( "puw_settings_main_notify_automatic", "puw_settings_main_notify_automatic_error", __( "Invalid automatic updates value entered", "hyperuix-wp-updater" ), "error" );
			}
			...
			if ( $sanitized_hide_updates <= 1 ) {
				...
				add_settings_error( "puw_settings_main_hide_updates", "puw_settings_main_hide_updates_error", __( "Invalid hide updates value entered", "hyperuix-wp-updater" ), "error" );
			}
		...
		public function send_test_email( $settings_errors ) {
			if ( isset( $settings_errors[0]['type'] ) && $settings_errors[0]['type'] == "success" ) {
				// $this->send_notification_email( __( "This is a test message from Hyperuix Updater.", "hyperuix-wp-updater" ) );
                $this->do_update_check();
			}
		}
		...
		public function puw_settings_main_field_cron_method() {
			...
			<select name="<?php echo self::OPT_FIELD; ?>[cron_method]">
				<option value="wordpress" <?php selected( $options['cron_method'], "wordpress" ); ?>><?php _e( "WordPress Cron", "hyperuix-wp-updater" ); ?></option>
				<option value="other" <?php selected( $options['cron_method'], "other" ); ?>><?php _e( "Other Cron", "hyperuix-wp-updater" ); ?></option>
			</select>
			<div>
				<br />
				<span class="description"><?php _e( "Cron Command: ", "hyperuix-wp-updater" ); ?></span>
		...
		public function puw_settings_main_field_notify_to() {
			...
			<span class="description"><?php _e( "Separate multiple email address with a comma (,)", "hyperuix-wp-updater" ); ?></span><?php
		}
		...
		public function puw_settings_main_field_notify_plugins() {
			$options = $this->getSetOptions( self::OPT_FIELD );
			?>
			<label><input name="<?php echo self::OPT_FIELD; ?>[notify_plugins]" type="radio" value="0" <?php checked( $options['notify_plugins'], 0 ); ?> /> <?php _e( "No", "hyperuix-wp-updater" ); ?>
			</label><br />
			<label><input name="<?php echo self::OPT_FIELD; ?>[notify_plugins]" type="radio" value="1" <?php checked( $options['notify_plugins'], 1 ); ?> /> <?php _e( "Yes", "hyperuix-wp-updater" ); ?>
			</label><br />
			<label><input name="<?php echo self::OPT_FIELD; ?>[notify_plugins]" type="radio" value="2" <?php checked( $options['notify_plugins'], 2 ); ?> /> <?php _e( "Yes but only active plugins", "hyperuix-wp-updater" ); ?>
			</label>
		<?php
		}
		...
		public function puw_settings_main_field_notify_themes() {
			$options = $this->getSetOptions( self::OPT_FIELD );
			?>
			<label><input name="<?php echo self::OPT_FIELD; ?>[notify_themes]" type="radio" value="0" <?php checked( $options['notify_themes'], 0 ); ?> /> <?php _e( "No", "hyperuix-wp-updater" ); ?>
			</label><br />
			<label><input name="<?php echo self::OPT_FIELD; ?>[notify_themes]" type="radio" value="1" <?php checked( $options['notify_themes'], 1 ); ?> /> <?php _e( "Yes", "hyperuix-wp-updater" ); ?>
			</label><br />
			<label><input name="<?php echo self::OPT_FIELD; ?>[notify_themes]" type="radio" value="2" <?php checked( $options['notify_themes'], 2 ); ?> /> <?php _e( "Yes but only active themes", "hyperuix-wp-updater" ); ?>
			</label>
		<?php
		}
		...
		public function puw_settings_main_field_notify_automatic() {
			$options = $this->getSetOptions( self::OPT_FIELD );
			?>
			<label><input name="<?php echo self::OPT_FIELD; ?>[notify_automatic]" type="checkbox" value="1" <?php checked( $options['notify_automatic'], 1 ); ?> /> <?php _e( "Yes", "hyperuix-wp-updater" ); ?>
			</label>
		<?php
		}
		...
		public function puw_settings_main_field_hide_updates() {
			...
			<select id="puw_settings_main_hide_updates" name="<?php echo self::OPT_FIELD; ?>[hide_updates]">
				<option value="1" <?php selected( $options['hide_updates'], 1 ); ?>><?php _e( "Yes", "hyperuix-wp-updater" ); ?></option>
				<option value="0" <?php selected( $options['hide_updates'], 0 ); ?>><?php _e( "No", "hyperuix-wp-updater" ); ?></option>
			</select>
		<?php
		}
